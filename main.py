...
# ═══════════════════════════════════════════════
# AIRPORT COORDINATE LOOKUP (ICAO → lat, lon)
# ═══════════════════════════════════════════════
...
def resolve_airport(value):
    """
    Accept either an ICAO string ('EGLL') or a [lat, lon] list/tuple.
    Always returns a (lat, lon) tuple.
    """
    if isinstance(value, str):
        coords = AIRPORT_COORDS.get(value.upper())
        if coords is None:
            raise ValueError(f"Unknown ICAO code: {value!r}. "
                             f"Known codes: {list(AIRPORT_COORDS.keys())}")
        return coords
    # Already a sequence [lat, lon]
...
if TORCH_AVAILABLE:
    ...
    # Load trained weights if available (route_optimizer.pt next to main.py)
    import os
    _weights_path = os.path.join(os.path.dirname(__file__), "route_optimizer.pt")
    if os.path.exists(_weights_path):
        try:
            model.load_state_dict(torch.load(_weights_path, map_location="cpu"))
            print(f"[MAVERICK] ✓ Trained weights loaded from {_weights_path}")
        except Exception as e:
            print(f"[MAVERICK] WARNING: could not load weights ({e}) — using random init")
    else:
        print("[MAVERICK] INFO: route_optimizer.pt not found — using random weights")
        print("[MAVERICK]       Run train_route_optimizer.py to train the model.")

    model.eval()
    print("[MAVERICK] PyTorch RouteOptimizer ready")
else:
    model = None
...
async def optimize_route(payload: dict):
    ...
    """
    # ── Resolve ICAO strings OR raw lat/lon arrays to (lat, lon) tuples
    try:
        origin = resolve_airport(payload.get("origin", [51.47, -0.46]))
        dest   = resolve_airport(payload.get("destination", [59.65, 17.92]))
    except ValueError as e:
        return {"error": str(e)}, 422

    # wind: frontend sends wind_direction + wind_speed_kt; convert to [wx, wy]
    wind_dir_deg = float(payload.get("wind_direction", 270))
    wind_spd_kt  = float(payload.get("wind_speed_kt",  18))
    wind_rad     = math.radians(wind_dir_deg)
    wx = math.sin(wind_rad) * wind_spd_kt
    wy = math.cos(wind_rad) * wind_spd_kt
    wind = [wx, wy]

    fw = float(payload.get("fuel_weight", 0.7))

    # Derive a fuel saving % from wind conditions (mirrors frontend physics)
    import math as _m
    bear_lat  = dest[0] - origin[0]
    bear_lon  = dest[1] - origin[1]
    bearing   = _m.atan2(bear_lon, bear_lat)
    wind_component = _m.cos(wind_rad - bearing) * wind_spd_kt
    aircraft_speed = 850  # km/h
    wind_factor    = 1 - (wind_component / aircraft_speed) * 0.4
    ai_saving_pct  = min(0.28, (0.17 + wind_spd_kt * 0.002) * max(0.5, wind_factor))
    ...
    # Override fuel estimate with physics-accurate calculation
    burn_rates = {"A320": 2.4, "B737": 2.6, "A380": 11.2, "B777": 6.8}
    aircraft   = payload.get("aircraft_type", "A320")
    burn_rate  = burn_rates.get(aircraft, 2.4)
    speed_kmh  = 850.0
    flight_hrs = dist_km / speed_kmh
    fuel_std   = flight_hrs * burn_rate
    fuel_opt   = fuel_std * (1 - ai_saving_pct)
    fuel_save  = fuel_std - fuel_opt
    co2_saved  = fuel_save * 2.54   # ICAO factor
    ...
    return {
        ...
        "fuel_standard_tonnes": round(fuel_std, 2),
        "fuel_estimate_tonnes": round(fuel_opt, 2),
        "fuel_saved_tonnes":    round(fuel_save, 2),
